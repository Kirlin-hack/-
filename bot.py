...
questions = [
    ...
    "3. Делитесь ли вы своими паролями с друзьями или близкими?",
    ...
    "7. Устанавливаете ли вы нелицензионное программное обеспечение?",
    "8. Проверяете ли настройки приватности аккаунтов?",
    "9. Отключаете ли вы антивирус, чтобы не мешал?",
    "10. Пользуетесь ли VPN?",
    "11. Знаете ли вы что такое фишинг?",
    "12. Даете ли вы приложениям все запрашиваемые разрешения, не глядя?",
    "13. Выходите ли из аккаунтов на чужих устройствах?",
    "14. Игнорируете ли вы подозрительные ситуации в интернете?",
    "15. Думаете ли перед публикацией личной информации?"
]

def calculate_score(question_index, answer):
    inverted_questions = [2, 6, 8, 11, 13]
    
    if question_index in inverted_questions:
        if answer == "Да":
            return 0
        elif answer == "Иногда":
            return 1
...
async def process_answer(message: Message, state: FSMContext):
    ...
    if message.text in ["Да", "Иногда", "Нет"]:
        points = calculate_score(q_index, message.text)
        score += points
